...
def get_cpu_count():
    """Get CPU count in a way that works across Python versions."""
    try:
        # Try multiprocessing approach first (works on Python 2.6+)
...
def run_test(target_ip, port, duration):
    ...
    for i in range(thread_count):
        thread = threading.Thread(
            target=lambda i=i: results.append(send_udp_packets(target_ip, port, duration, packet_size, i))
        )
